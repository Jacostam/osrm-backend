                               ...
                               std::string &csv_filename)
{
    ...
    options.add_options()("help,h", "Show this help message")(
        "input,i",
        boost::program_options::value<std::string>(&osm_filename),
                                ...
                                std::int64_t &restriction_value)
{
    ...
    options.add_options()("help,h", "Show this help message")(
        "input,i",
        boost::program_options::value<std::string>(&input_filename),
        "Input conditional restrictions file in CSV format")(
        "output,o",
        boost::program_options::value<std::string>(&output_filename),
        "Output conditional restrictions file in CSV format")(
        "tz-shapes,s",
        boost::program_options::value<std::string>(&tz_filename)->default_value(tz_filename),
        "Timezones shape file without extension")(
        "utc-time,t",
        boost::program_options::value<std::time_t>(&utc_time)->default_value(utc_time),
        "UTC time-stamp [default=current UTC time]")(
        "value,v",
        boost::program_options::value<std::int64_t>(&restriction_value)
            ->default_value(restriction_value),
...
class ConditionalSpeedLimitsCollector : public osmium::handler::Handler
{
  ...
  public:
    ...
    void way(const osmium::Way &way)
    {
        ...
        for (; first != last; ++first)
        {
            ...
            for (auto &speed_limit : parsed)
            {
                ...
                // TODO handle complete way and one-directional flag
                speed_limits.push_back(ConditionalSpeedLimit{way.nodes()[0].ref(),
                                                             way.nodes()[1].ref(),
                                                             first->key(),
                                                             speed_limit_value,
    ...
    template <typename Callback> void process(Callback callback)
    {
        ...
        {
            // TODO check a mid-point as a location
            const auto &location = location_storage.get(
